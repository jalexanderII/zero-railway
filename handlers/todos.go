...
func HandleAllTodos(c *fiber.Ctx) error {
	...
	// return todos
	return c.Status(fiber.StatusOK).JSON(fiber.Map{"status": "success", "data": todos})
}
...
func HandleCreateTodo(c *fiber.Ctx) error {
	...
	if err := c.BodyParser(nTodo); err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"status": "error", "message": "bad input", "data": err.Error()})
	}
	...
	return FiberJsonResponse(c, fiber.StatusOK, "success", "inserted_id", res.InsertedID)
}
